...
class Program
{
    ...
    {
        Ray ray = new Ray(new Vector3(2,3,4), new Vector3(0.577f,0.577f,0.577f));
        RayTracer.Engine.Geometries.Plane plane = new (new Vector3(1,0,0), -7);
        float t;
    ...
    static void TestIntersections()
    {
        ...
        RayTracer.Engine.Geometries.Plane plane1 = new (direction1, 0);
        for (int y = -5; y <= 5; y++)
        {
            for (int x = -5; x <= 5; x++)
            {
    ...
    static void Main(string[] args)
    {
        ...
        SceneObject plane1 = new SceneObject(new RayTracer.Engine.Geometries.Plane(new Vector3(0, 0, 1), 8), new Material(new Finish(), new Solid(new Color(1, 1, 1))));
        ...
        using (Stream stream = new FileStream(@"c:\temp\dump.tga", FileMode.Create))
